...
const Survey5 = ({ navigation, }) => {
    ...
    const image_shoe = require('../../../../../assets/denim-jacket.png');
        ...
        return (
            <TouchableOpacity style={{ width: '25%', aspectRatio: 1 / 1.4, justifyContent: 'center', alignItems: 'center', }} onPress={() => {
                if (!istouched) {
                    ...
                    setIstouched(false);
                    setNeedlist(needlist.filter((shoe) => shoe !== classify))
                }
            }}>
                <View style={{
                ...
                <View style={{ flex: 1, justifyContent: 'flex-start', alignItems: 'flex-start' }}>
                    ...
                    <View style={{ flex: 1, width: '100%', alignItems: 'center', backgroundColor: '#ffffff' }}>
                        <View style={{ width: '100%', marginVertical: 20, justifyContent: 'center', alignItems: 'center', }}>
                            <Text style={{ width: '80%', fontSize: 35, fontFamily: 'NotoSansKR-Medium', includeFontPadding: false, }}>원하는 신발 종류</Text>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false} style={{ width: '100%', }} contentContainerStyle={{ justifyContent: 'flex-start', alignItems: 'center', }}>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-evenly', alignItems: 'center', width: '100%' }} >
                                <Category istouched={istouched1} setIstouched={setIstouched1} classify='구두' image={image_shoe} />
                                <Category istouched={istouched2} setIstouched={setIstouched2} classify='워커/부츠' image={image_shoe} />
                                <Category istouched={istouched3} setIstouched={setIstouched3} classify='운동화' image={image_shoe} />
                            </View>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-evenly', alignItems: 'center', width: '100%' }} >
                                <Category istouched={istouched4} setIstouched={setIstouched4} classify='샌들/슬리퍼' image={image_shoe} />
                                <TouchableOpacity style={{ width: '25%', aspectRatio: 1 / 1.4, justifyContent: 'center', alignItems: 'center', }} onPress={() => {
                                    if (!istouched5) {
                                        setIstouched5(true);
                                        setIstouched1(false);
                                        ...
                                        setIstouched5(false);
                                        setNeedlist(needlist.filter((shoe) => shoe !== '알아서 해주세요!'))
                                    }
                                }}>
                                    <View style={{
                                        width: '100%', aspectRatio: 1 / 1, backgroundColor: istouched5 === true ? '#d2d2d2' : '#ffffff',
                                        borderWidth: 1, borderColor: '#d2d2d2', borderRadius: 100, justifyContent: 'center', alignItems: 'center',
                                    }}>
                                ...
                                <TouchableOpacity style={{ width: '25%', aspectRatio: 1 / 1.4, justifyContent: 'center', alignItems: 'center', }} onPress={() => {
                                    if (!istouched6) {
                                        ...
                                        setIstouched6(false);
                                        setNeedlist(needlist.filter((shoe) => shoe !== '필요없어요!'))
                                    }
                                }}>
                                    <View style={{
                            ...
                            <View flexDirection='row' style={{ width: '100%', marginTop: 10 }}>
                                <TouchableOpacity style={{ width: '100%', }} onPress={() => {
                                    if (!istouched7) {
                                        ...
                                        setEtc('');
                                        setIstouched7(false);
                                        setNeedlist(needlist.filter((shoe) => shoe !== '기타'))
                                    }
                                }}>
                        ...
                        <FloatingButton title='다음' navigation={navigation} index='Survey6' category='need_shoes' contents={contents} next={next} />
                    </View>
                </View>
